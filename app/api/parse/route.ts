...
export async function POST(req: Request) {
    try {
        ...
        // Persistent rate limiting (max 30 requests per hour by phone or IP)
        ...
        try {
            const isAllowed = await db.runTransaction(async (transaction: any) => {
            ...
            console.warn('Rate limiter warning (allowing request):', rlErr);
        }
            ...
            try {
                ...
                for (const modelName of candidateModels) {
                    try {
                        ...
                        console.warn(`Model ${modelName} parse failed:`, mErr?.message);
                    }
                }
            } catch (aiErr) {
